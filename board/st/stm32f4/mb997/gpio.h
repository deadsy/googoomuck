...
static inline uint32_t debounce_input(void) {
	...
	return ((gpio_rd(LIMIT_X) << X_LIMIT_BIT) | (gpio_rd(LIMIT_Y) << Y_LIMIT_BIT) | (gpio_rd(LIMIT_Z) << Z_LIMIT_BIT) | (gpio_rd(LIMIT_A) << A_LIMIT_BIT) | (gpio_rd(SWITCH_E_STOP) << CONTROL_RESET_BIT) | (gpio_rd(PUSH_BUTTON) << PUSH_BUTTON_BIT));
}
...
static inline int coolant_flood_is_on(void) {
	return 0;
}
...
static inline int coolant_mist_is_on(void) {
	return 0;
}
...
static inline int spindle_is_on(void) {
	return 0;
}
static inline int spindle_is_fwd(void) {
	return 0;
}
